        ...
        (let [tx-report (db/transact! [(dl/new-entity {:db/id "new-board"} :board)
          ...
          (testing "alice and bob can read the chat"
            (is (= [[alice-id "hello1"]
                    [alice-id "hello2"]
                    [bob-id "hello3"]]
                   (->> (cd/chat (az/with-account-id! {:account (db/entity alice-account)}
                                   {:other-id (sch/wrap-id (db/entity bob))}))
                        :chat/messages
                        (map (juxt (comp :entity/id :entity/created-by) :chat.message/content)))
                   (->> (cd/chat (az/with-account-id! {:account (db/entity bob-account)}
                                   {:other-id (sch/wrap-id (db/entity alice))}))
                        :chat/messages
                        (map (juxt (comp :entity/id :entity/created-by) :chat.message/content))))))
          (testing "eve cannot read the the chat"
            (is (= []
                   (->> (cd/chat (az/with-account-id! {:account (db/entity eve-account)}
                                   {:other-id (sch/wrap-id (db/entity bob))}))
                        :chat/messages
                        (map (juxt (comp :entity/id :entity/created-by) :chat.message/content)))
                   (->> (cd/chat (az/with-account-id! {:account (db/entity eve-account)}
