...
(defn ref
  ...
  ([cardinality ks]
   (let [nested? (= 1 (count ks))
         leaf-type (if nested?
                     [:multi {:dispatch 'map?}
                      [true [:ref (keyword "entity" (namespace (first ks)))]]
                      [false (lookup-ref ks)]]
                     (lookup-ref ks))]
...
(defn infer-db-type [m]
  (merge (when (and (s- m) (not (:db/type m)))
...
(comment
 (->> sb-schema
      (remove (comp :db/valueType val))
      (remove (comp #{"entity"} name key))
      (into {})))
...
(def sb-schema
  (-> {:action/policy {:doc "A policy describes how an action is restricted"
       ...
       :board.project/fields (merge (ref :many #{:field.spec/id})
                                    {s- [:sequential :entity/field.spec]})
       ...
       :entity/discussion {s- [:map {:closed true}
                               ...
                               :discussion/project
                               :ts/created-at]}
       :entity/domain (merge (ref :one #{:domain/name})
       ...
       :entity/field.spec {:doc "Description of a field."
                           ...
                           s- [:map {:closed true}
                               ...
                               :field.spec/order
                               :field/type]}
       :entity/firebase-account {s- [:map {:closed true}
                                     :firebase-account/id
                                     :firebase-account/email]}
       :entity/grant {s- [:and [:map {:closed true}
                                (? :grant/member)
       ...
       :grant/roles (merge s/keyword
                           s/many
                           {:doc "Set of roles granted",
                            s- [:set :grant/role]})
       ...
       :content/fields (merge (ref :many #{:field/id})
                              s/component)
       ...
       :member/tags.custom {s- [:sequential :map/tag.custom]}
       ...
       :post/_discussion {s- [:sequential :entity/post]}
       ...
       :tag/restricted? {:doc "Tag may only be modified by an admin of the owner of this tag"
                         s- :boolean}
       :tag.custom/label {s- :string}
       :map/tag.custom {s- [:map :tag.custom/label]}
...
(def entity-schemas (into {} (map (fn [k] [k (keyword "entity" (namespace k))])) id-keys))
