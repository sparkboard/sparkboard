(ns sparkboard.views.project
  (:require #?(:clj [sparkboard.datalevin :as dl])
            #?(:clj [sparkboard.endpoints :as endpoints])
            [promesa.core :as p]
  ...
  (ui/with-form [!project {:entity/title ?title}]
    [:div
     ...
     [:button {:on-click #(p/let [res (routes/POST `db:new! @!project)]
                            (when-not (:error res)
                              (routes/set-path! ['sparkboard.views.board/read params])))}
      (tr :tr/create)]]))
   ...
   (defn db:read
     {:endpoint  {:query ["/p/" ['uuid :project-id]]}
      :authorize (fn [req {:as params :keys [project-id]}]
                   (endpoints/member:read-and-log! project-id (:db/id (:account req)))
                   params)}
     [{:keys [project-id]}]
