(ns sparkboard.views.org
  (:require #?(:clj [sparkboard.datalevin :as dl])
            #?(:clj [sparkboard.endpoints :refer [member:read-and-log!]])
...
(ui/defview read
  ...
  (forms/with-form [_ ?q]
    (let [{:as   org
           :keys [entity/description]} (ws/use-query! ['sparkboard.views.org/db:read params])
          q      (ui/use-debounced-value (u/guard @?q #(> (count %) 2)) 500)
          result (ws/use-query ['sparkboard.views.org/db:search {:org-id (:org-id params)
                                                                  :q      q}])]
      [:div
       (ui/entity-header org
                         [ui/header-btn [icons/settings]
                          (routes/path-for 'sparkboard.views.org/edit params)]
                         #_[:div
                         ...
                         [:a.btn.btn-light {:href (routes/href 'sparkboard.views.board/new
...
(ui/defview edit
  ...
  [{:as params :keys [org-id]}]
  (let [org (ws/use-query! '[sparkboard.views.org/db:edit params])]
