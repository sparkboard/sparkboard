...
(query/defquery db:chats
  {:prepare [az/with-account-id!]}
  [{:as params :keys [account-id entity-id]}]
  (let [clauses (cond-> [[:chat/participants account-id]]
                        (not= entity-id account-id)
                        (conj [:chat/entity entity-id]))]
    (->> (db/where clauses)
...
(query/defquery db:counts
  {:prepare [az/with-account-id!]}
  [params]
  (let [chats @(query/$ db:chats params)]
    {:total  (count chats)
     :unread (->> chats
...
(ui/defview sidebar [{:as             params :keys [account-id
                                                    entity-id]
    ...
    (doall
      (for [{:as   chat
        ...
        [:a.flex.flex-col.py-2.cursor-default.mx-1.px-1
         {:href  (routes/path-for `chat {:entity-id entity-id
                                         :chat-id   id})
          :key   id
         ...
         [:div.flex
          [:div.pl-4.line-clamp-2.text-gray-700
           (ui/show-prose
...
(ui/defview with-sidebar [params child]
  [:div.flex.divide-x.h-screen
   [:div.overflow-y-scroll {:class ["min-w-48 max-w-md w-1/3"]}
    [sidebar params]]
   [:div.flex-auto.px-2 child]])
...
(ui/defview chats
  {:route "/chats"}
  [{:as params :keys [other-id chat-id entity-id account-id]}]
  (let [params (assoc params :entity-id (or entity-id account-id))]
    [with-sidebar params
...
(routes/register-route entity-chats
  {:alias-of chats
   :route    ["/chats/" ['entity/id :entity-id]]})

(routes/register-route chat
  {:alias-of chats
   :route    ["/chats/" ['entity/id :entity-id] "/" ['entity/id :chat-id]]})
